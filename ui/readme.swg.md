...
# SmartSuggest
When collecting the suggestions, the SmartSuggest module follows a confidence based staging approach. The next stage (lower confidence) is carried out if the previous one(s) did not provide the number of suggestions requested. As an example, this could look like this for a request to return 10 suggestions:
...
If the first stage alredy finds >= 10 suggestions, we directly return them. If < 10 suggestions are found, the next stage is executed until the number of requested suggestions has been found.

## Basic settings - Fuzziness
Controls the proportion of fuzzy matches in the overall result. The default is set to __precision__ where the module won't search for additional *Fuzzy-Matches* if the confident-stage(s) beforhand found any hits, even if the number of found matches is less than the requested number of suggestions. If you set the setting to __recall__, additional *Fuzzy-Matches* will be considered until the number of requested suggestions is reached.
